...
data_date <- "7_16_2023" #date of dataset - must use dataset from April 18, 2023 unless you want to update every single number/estimate

dir.root <- paste0("C:/Users/", Sys.getenv("USERNAME"),
                   ".CDC",
                   "/OneDrive - CDC/Kenya Recency Coding/")

# Create export directories for saving files
import.dir <- paste0(dir.root, "Inputs/")
export.dir <- paste0(dir.root,"Outputs/")
dir.create(paste0(export.dir, Sys.Date))
export.folder <- paste0(export.dir, "/")

#Load data
rec_data <- read.csv(paste0(import.dir, "Kenya RS Data_as of 7.16.2023", ".csv"), stringsAsFactors = FALSE, encoding="UTF-8")
```
...
#rec_data1 <- rec_data1 %>% rename(recency_id=ï..recency_id)
...
#Time since last test
        ...
        #Combine categories for IAS Abstract Table
           valid_rec_ayp <- valid_rec_ayp %>%
              mutate(last_tested.recode = 
                       case_when(last_tested_hiv_before_encounter == "Tested 13 – 24 months ago" | last_tested_hiv_before_encounter == "Tested more than 2 years ago" ~ "Tested more than 1 year ago",
    ...
    lasttest <- c("Tested <6 months ago","Tested 6-12 months ago", "Tested 13 – 24 months ago", "Tested more than 2 years ago")
    status <- c("Longterm", "Recent")
